...
if "sample" in datafile:
	nummap = {ind: string.ascii_uppercase[b.n] for ind, b in enumerate(bricks)}
else:
...
#TODO figure out how many bricks will fall
"""
for each brick, find unsupported bricks above

NOTE figure out caching later

for each brick
	create set of destroyed bricks
	iterate through above bricks (in order)
		if this brick was supported fully by set of destroyed bricks, add to set
		count unsupported bricks above that, etc
"""

#TODO skip those in part 1

bricks2break = [0]*len(bricks) #TODO 

for basebrick in range(len(bricks)):
	destroyedbricks = set([basebrick])
	for abovebrick in range(len(bricks)):#range(basebrick+1, len(bricks)): #don't worry about bricks below
		if len(bricksbelow[abovebrick]) and all([b in destroyedbricks for b in bricksbelow[abovebrick]]):
			destroyedbricks.add(abovebrick)

	bricks2break[basebrick] = len(destroyedbricks)-1
...
for ind, b2b in enumerate(bricks2break):
	print(nummap[ind], b2b)

print(sum(bricks2break))
